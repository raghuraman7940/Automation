package img.auto.test;

import img.auto.xls.read.Xls_Reader;
...
public class DriverScript {
	...
	public static void main(String[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, IOException, NoSuchMethodException, SecurityException {
		FileInputStream fs = new FileInputStream(System.getProperty("user.dir")+"\\src\\test\\resources\\img\\auto\\config\\config.properties");
		
		CONFIG= new Properties();
		CONFIG.load(fs);
		
		fs = new FileInputStream(System.getProperty("user.dir")+"\\src\\test\\resources\\img\\auto\\config\\or.properties");
		OR= new Properties();
		OR.load(fs);
	...
	public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		...
		suiteXLS = new Xls_Reader(System.getProperty("user.dir")+"\\src\\test\\resources\\img\\auto\\xls\\Suite.xlsx");
		...
		for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++){
			...
			if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES)){
				...
				currentTestSuiteXLS=new Xls_Reader(System.getProperty("user.dir")+"\\src\\test\\resources\\img\\auto\\xls\\"+currentTestSuite+".xlsx");
